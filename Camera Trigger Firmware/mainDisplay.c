...
static void editContrast() {
    ...
    numberEditor_initInt(oldValue, handleContrast, mainDisplay_openMenu, "Contrast Setting\0",
...
static void editBrightness() {
    ...
    numberEditor_initInt(oldValue, handleBrightness, mainDisplay_openMenu, "Brightness Setting\0",
                         "Value", BRIGHTNESS_MIN, BRIGHTNESS_MAX);
}

static void handleEventCount(uint32_t value) {
    // eepromDriver_writeDword(GLOBALVAR_TIMING_SET_COUNT, value);
    events_updateEventCount(value);
    mainDisplay_openMenu();
}

static void editEventCount() {
    uint32_t oldValue = (uint32_t)eepromDriver_readDword(GLOBALVAR_TIMING_SET_COUNT);
    numberEditor_initInt(oldValue, handleEventCount, mainDisplay_openMenu, "Trigger Event Count\0",
                         "Triggers", SET_COUNT_MIN, SET_COUNT_MAX);
}
