...
ring_buffer_size_t usart0_recv_queue_size(void) {
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { result = ring_buffer_num_items(&usart0_recv_ring_buf); }
    return result;
}
...
ring_buffer_size_t usart0_recv_dequeue(char *data) {
    ring_buffer_size_t result;
    /* Prevent race conditions */
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE) { result = ring_buffer_dequeue(&usart0_recv_ring_buf, data); }
    return result;
}
...
usart_desc_t *get_usart0_descriptor(void) {
    static usart_desc_t descriptor = {.usart_recv_queue_size = usart0_recv_queue_size,
